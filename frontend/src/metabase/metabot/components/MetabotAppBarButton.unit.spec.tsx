...
describe("MetabotAppBarButton", () => {
  ...
  it("should not render the button on the full-page AI exploration (/question/ask) surface", async () => {
    setup({ isMetabotEnabled: true, pathname: "/question/ask" });
    // Wait for the permissions request so we know the button would otherwise render
    await waitFor(() => {
      expect(fetchMock.callHistory.calls().length).toBeGreaterThan(0);
    });
    expect(
  ...
  it("should still render the button on other question pages", async () => {
